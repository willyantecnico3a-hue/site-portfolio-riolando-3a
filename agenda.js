...
// =====================================================
// CORREÇÃO FORÇADA - FORMULÁRIO DE EVENTO EM MODAL SUSPENSO
// Esta correção move o formEvento para uma janela sobreposta,
// mesmo que ele ainda esteja dentro do modalDia no HTML.
// =====================================================

let modalEventoFlutuante = null;
let caixaModalEventoFlutuante = null;

document.addEventListener("DOMContentLoaded", function () {
    criarModalFlutuanteDeEvento();
});

function criarModalFlutuanteDeEvento() {
    const formulario = document.getElementById("formEvento");

    if (!formulario) {
        console.log("formEvento não encontrado para criar modal flutuante.");
        return;
    }

    // Se o modal já existir, não cria de novo.
    if (document.getElementById("modalEventoFlutuante")) {
        return;
    }

    modalEventoFlutuante = document.createElement("div");
    modalEventoFlutuante.id = "modalEventoFlutuante";
    modalEventoFlutuante.className = "modal-evento-flutuante";

    caixaModalEventoFlutuante = document.createElement("div");
    caixaModalEventoFlutuante.className = "caixa-modal-evento-flutuante";
    ...
    topo.innerHTML = `
        <div>
            ...
            <p>Cadastre ou edite o evento sem precisar descer a tela.</p>
        </div>
    ...
    caixaModalEventoFlutuante.appendChild(topo);

    // Move o formulário original para dentro do modal flutuante.
    formulario.style.display = "block";
    caixaModalEventoFlutuante.appendChild(formulario);

    modalEventoFlutuante.appendChild(caixaModalEventoFlutuante);
    document.body.appendChild(modalEventoFlutuante);

    modalEventoFlutuante.addEventListener("click", function (event) {
        if (event.target === modalEventoFlutuante) {
            fecharModalEventoFlutuante();
        }
    });

    console.log("Modal flutuante de evento criado e formulário movido com sucesso.");
}
...
// =====================================================
// SOBRESCREVE A FUNÇÃO DE EDITAR PARA ABRIR NO MODAL
// =====================================================
...
// =====================================================
// FAZ O BOTÃO "+ CRIAR EVENTO" ABRIR NO MODAL FLUTUANTE
// =====================================================

const botaoCriarEventoCorrigido = document.getElementById("btnAbrirFormEvento");

if (botaoCriarEventoCorrigido) {
    botaoCriarEventoCorrigido.addEventListener("click", function () {
        setTimeout(function () {
            if (!perfilUsuario || perfilUsuario.funcao !== "admin") {
                return;
            }
...
// =====================================================
// FECHA O MODAL FLUTUANTE COM ESC
// =====================================================
